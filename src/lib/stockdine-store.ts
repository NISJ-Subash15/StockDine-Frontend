...
export const stockDineStore = {
  ...
  updateUserProfile: async (data: { name: string; mobile?: string; email?: string; avatar?: string; profilePhoto?: string }): Promise<{ success: boolean; user?: any; message?: string }> => {
    try {
      ...
      const res: any = await api.auth.updateCustomerProfile({
        ...
        avatar: (data.avatar || data.profilePhoto || "").trim(),
      });

      if (res && res.success && (res.user || res.profile || res.customer)) {
        const serverUser = res.user || res.profile || res.customer;
        const updatedProf = {
          ...(currentAuthSession.profileData || {}),
          ...serverUser,
        };
